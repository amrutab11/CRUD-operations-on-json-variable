...
router.get('/:id', (req, res) => {
    const found = members.some(member => member.id === parseInt
        (req.params.id));

    if(found)
    {
        res.json(members.filter(member => member.id === parseInt
            (req.params.id)));
    } else {
...
router.post('/', (req, res) => {
    ...
    if(!newMember.name || !newMember.email){
        return res.status(400).json({msg: "name and email is empty"});
    }
